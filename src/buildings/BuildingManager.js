Settlement.BuildingManager=class{
 ...
 /* Full teardown: grid, jobs, homes, farm plots, tower cooldowns and render
    state. Claimed territory is deliberately NOT revoked - land already secured
    stays secured. */
 demolish(id){
  let b=this.byId(id);if(!b)return false;let g=this.game;
  for(const c of g.citizens.list){
   if(c.workplace===b.id){c.workplace=null;c.job="Unassigned";c.path=[];c.pathIndex=0}
   if(c.home===b.id)c.home=null;
  }
  g.farms?.plots?.delete(b.id);
  g.towers?.cool?.delete(b.id);
  this.remove(id);
  g.housing?.reconcile();
  g.bus.emit("citizens:changed");
  g.bus.emit("building:removed",b);
  g.bus.emit("toast","🗑️ "+(Settlement.BuildingDefs[b.type]?.name||"Structure")+" removed.");
  return true;
 }
