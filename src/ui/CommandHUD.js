...
(()=>{
 ...
 p.renderShell=function(){baseShell.call(this);let root=this.root;if(!root.querySelector("#hud-orb")){let b=document.createElement("button");b.id="hud-orb";b.type="button";b.dataset.act="commandHudToggle";b.setAttribute("aria-label","Open resource ledger");b.innerHTML=`⚜<span class="hud-arrow">+</span>`;root.appendChild(b)}};
 p.commandHudRender=function(){
  if(!mobile())return;let bar=document.querySelector("#topbar"),orb=document.querySelector("#hud-orb"),quest=document.querySelector("#questbox");if(!bar||!orb)return;if(quest){quest.classList.add("command-hud-quest-removed");quest.setAttribute("aria-hidden","true")}
  let g=this.game,r=g.resources.v,max=g.xp.isMax?.()||false,nxt=g.xp.next(),pct=max?100:(g.xp.progressPct?.()??Math.min(100,g.xp.xp/nxt*100)),open=pref();
