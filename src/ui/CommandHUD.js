...
/* Per-resource storage clarity. Capacity is independent; Gold remains uncapped. */
(()=>{let P=Settlement.UIManager?.prototype;if(!P||P.__perResourceHUD)return;P.__perResourceHUD=true;let base=P.commandHudRender;P.commandHudRender=function(){base?.call(this);let cap=this.game.resources.capacity(),rows=document.querySelectorAll("#topbar .ledger-row"),keys=Object.keys(Settlement.ResourceDefs||{}).filter(k=>k!=="xp");rows.forEach((row,i)=>{let k=keys[i],b=row.querySelector("b");if(!b||!this.game.resources.isStoredResource(k))return;let n=Math.floor(this.game.resources.v[k]||0),full=n>=cap;b.textContent=`${n.toLocaleString()} / ${Math.floor(cap).toLocaleString()}${full?" • FULL":""}`;row.classList.toggle("resource-full",full)});if(!matchMedia("screen and (max-width:600px)").matches){let pills=document.querySelectorAll("#topbar .resource-pill");["gold","wood","stone","food"].forEach((k,i)=>{if(k==="gold"||!pills[i])return;let n=Math.floor(this.game.resources.v[k]||0),b=pills[i].querySelector("b");if(b)b.textContent=`${n}/${Math.floor(cap)}${n>=cap?" FULL":""}`})}}})();
