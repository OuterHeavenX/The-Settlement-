...
export class GothicTerrainFoundation{
 ...
 irregularPlane(w,d,mat,seed,y=.12){const g=new THREE.CircleGeometry(1,14),a=g.attributes.position;for(let i=1;i<a.count;i++){const ang=Math.atan2(a.getY(i),a.getX(i)),j=.86+(hash(seed,i)%100)/100*.18;a.setXYZ(i,Math.cos(ang)*j*w*.5,Math.sin(ang)*j*d*.5,0)}g.rotateX(-Math.PI/2);const m=new THREE.Mesh(g,mat);m.position.y=y;m.rotation.y=(hash(seed,91)%628)/100;m.renderOrder=1;return m}
 ...
 rebuild(){const b=this.territoryBounds();if(!b)return;const roads=this.roads(b),sig=[Math.round(b.minX),Math.round(b.maxX),Math.round(b.minZ),Math.round(b.maxZ),...roads.map(r=>r.id+":"+r.x+","+r.y)].join("|");if(sig===this.sig)return;this.sig=sig;this.clear();const cx=(b.minX+b.maxX)/2,cz=(b.minZ+b.maxZ)/2,w=b.maxX-b.minX,d=b.maxZ-b.minZ,base=new THREE.Mesh(new THREE.PlaneGeometry(w,d).rotateX(-Math.PI/2),this.baseMat);base.position.set(cx,.04,cz);this.root.add(base);for(let i=0;i<26;i++){const h=hash(i,Math.round(cx+cz)),x=b.minX+(h%1000)/1000*w,z=b.minZ+((h>>>10)%1000)/1000*d,p=this.irregularPlane(T*(1.5+(h%90)/100),T*(1.2+((h>>>7)%90)/100),i%4===0?this.dampMat:this.soilMat,h,.1+i*.0002);p.position.x=x;p.position.z=z;this.root.add(p)}for(const r of roads){const x=(r.x+.5)*T,z=(r.y+.5)*T,h=hash(r.x,r.y),p=this.irregularPlane(T*1.18,T*1.12,this.roadMat,h,.42);p.position.x=x;p.position.z=z;this.root.add(p);const wear=this.irregularPlane(T*.42,T*1.02,this.wearMat,h^991,.44);wear.position.x=x;wear.position.z=z;this.root.add(wear)}}
 update(){this.rebuild()}
}
