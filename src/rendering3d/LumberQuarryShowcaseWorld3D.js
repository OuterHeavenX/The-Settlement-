...
export class LumberQuarryShowcaseWorld3D extends CompleteSettlementWorld3D{
 buildStructure(b){
  const g=super.buildStructure(b);if(!b?.complete||(b.type!=="lumber"&&b.type!=="quarry"))return g;
  const m=this.industryMaterials(),w=b.w*T,d=b.h*T,t=this.tierOf(b.level||1);
 ...
 polishLumber(g,w,d,t,m,b){
  ...
  this.part(this.box(w*.96,1.3,d*.92),m.soil,0,1,d*.01,g);
  ...
  // Tall organized log racks: intentionally larger than the original clutter piles.
  for(const side of[-1,1]){
   const rx=side*w*.34;for(const z of[-d*.18,d*.08]){
  ...
  this.part(this.box(w*.32,6,14),m.timber,0,10,d*.3,g);
  for(const x of[-w*.12,w*.12]){const leg=this.part(this.box(4,18,4),m.timber,x,7,d*.3,g);leg.rotation.z=x<0?.25:-.25}
 ...
 polishQuarry(g,w,d,t,m,b){
  // Exposed excavation floor and stepped retaining faces.
  this.part(this.box(w*.96,2,d*.9),m.ash,0,1,d*.01,g);
  ...
  for(let i=0;i<10+t*2;i++){
   const side=i%2?-1:1,x=side*(w*.27+(i%3)*7),z=-d*.12+(i%5)*d*.13;
  ...
  // Cut-stone staging stacks and a second heavy cart silhouette.
  for(let i=0;i<3+t;i++)this.part(this.box(17,8,14),m.stone,w*.12+(i%2)*18,4+Math.floor(i/2)*9,d*.34,g);
  const cartX=w*.29,cartZ=-d*.31;this.part(this.box(31,9,19),m.timber,cartX,9,cartZ,g);
