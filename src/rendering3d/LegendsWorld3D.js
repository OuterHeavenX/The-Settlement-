...
export class LegendsWorld3D extends PermanentDayWorld3D{
 buildStructure(b){if(b?.complete&&b.type==="hallOfLegends")return this.makeHallOfLegends(b);return super.buildStructure(b)}
 makeHallOfLegends(b){
  ...
  // Twin archer galleries make the building's defensive purpose readable.
  ...
  // Trophy plinths along the front courtyard.
  for(let i=0;i<5;i++){let x=-80+i*40;box(20,12,20,stone2,x,15,-126);box(4,20,4,gold,x,31,-126)}
  g.position.set((b.x+b.w/2)*T,0,(b.y+b.h/2)*T);g.userData.buildingId=b.id;g.userData.legendHall=true;return g
 }
}
