...
export class MasonShowcaseWorld3D extends LumberQuarryShowcaseWorld3D{
 masonGeo(key,factory){const k="mason:"+key;if(!this.geoCache.has(k))this.geoCache.set(k,factory());return this.geoCache.get(k)}
 rock(size){return this.masonGeo("rock:"+size,()=>new THREE.DodecahedronGeometry(size,0))}
 cylinder(key,r1,r2,h,segments){return this.masonGeo("cyl:"+key,()=>new THREE.CylinderGeometry(r1,r2,h,segments))}
 buildStructure(b){
  ...
  this.polishMason(g,w,d,t,m);
  g.position.set((b.x+b.w/2)*T,0,(b.y+b.h/2)*T);
  return g
 }
 polishMason(g,w,d,t,m){
  ...
  // Raw stone yard: deliberately irregular and visually separate from dressed storage.
  const rawRoot=new THREE.Group();g.add(rawRoot);
  for(let i=0;i<7+t;i++){const size=7+(i%3)*2,rock=this.part(this.rock(size),i%4===0?stoneDark:stone,-w*.31+(i%3)*12,6+(i%2)*3,d*.23+Math.floor(i/3)*12,rawRoot);rock.scale.set(1.1,.7+(i%2)*.18,.9+(i%3)*.12);rock.rotation.y=i*.61}
  for(let i=0;i<2+t;i++){const slab=this.part(this.box(25,5,14),stoneDark,-w*.34+i*5,3+i*5,d*.39,g);slab.rotation.y=i%2?-.08:.06}
  for(let i=0;i<3;i++){const roller=this.part(this.cylinder("roller",3.2,3.2,28,8),timber,-w*.12+i*11,4,d*.39,g);roller.rotation.z=Math.PI/2}
  ...
  const roughBlock=this.part(this.rock(10),stone,0,5,0,workRig),dressedBlock=this.part(this.box(21,13,17),stone,0,5,0,workRig);dressedBlock.visible=false;
  const hammer=this.part(this.box(3,24,3),timber,14,20,0,workRig);this.part(this.box(13,6,6),iron,0,10,0,hammer);
  const chisel=this.part(this.box(2.3,22,2.3),iron,-8,17,1,workRig);chisel.rotation.z=.28;workRig.visible=false;
  ...
  this.part(this.cylinder("brazier",8,6,9,8),iron,-w*.25,7,-d*.05,g);
  this.part(this.box(8,13,8),lantern,-w*.25,17,-d*.05,g);
  ...
  g.userData.masonShowcase={workRig,roughBlock,dressedBlock,hammer,chisel,lanternMat:lantern};
 }
}
