...
async function startWebGL(game,canvas){
 try{
  const{AmenityStableWorld3D}=await import("./AmenityStableWorld3D.js?v=0.12.0-day-weather-defense");
  const world=new AmenityStableWorld3D(game,canvas);
  if(await Promise.resolve(world.init()))return world;
 }catch(e){console.error("The Settlement: stable permanent-day 3D layer failed; restoring proven living-world renderer",e)}
 try{
...
async function boot(){
 ...
 console.info(`The Settlement: optional Three.js ${backend.toUpperCase()} permanent-day weather presentation active`);
}
boot();
