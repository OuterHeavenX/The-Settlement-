...
async function startWebGL(game,canvas){let MilitaryFortificationWorld3D;({MilitaryFortificationWorld3D}=await import("./MilitaryFortificationWorld3D.js"));const world=new MilitaryFortificationWorld3D(game,canvas);return(await Promise.resolve(world.init()))?world:null}
...
async function boot(){
 ...
 if(!world&&gl){try{world=await startWebGL(game,canvas);if(world)backend="webgl"}catch(e){console.error("The Settlement: local WebGL fortification presentation unavailable",e)}}
