...
async function startWebGL(game,canvas){let CivicCommerceWorld3D;({CivicCommerceWorld3D}=await import("./CivicCommerceWorld3D.js"));const world=new CivicCommerceWorld3D(game,canvas);return(await Promise.resolve(world.init()))?world:null}
...
async function boot(){
 ...
 if(!world&&gl){try{world=await startWebGL(game,canvas);if(world)backend="webgl"}catch(e){console.error("The Settlement: local WebGL civic commerce presentation unavailable",e)}}
