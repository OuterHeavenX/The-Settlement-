...
async function startWebGL(game,canvas){let ResidentialDistrictWorld3D;({ResidentialDistrictWorld3D}=await import("./ResidentialDistrictWorld3D.js"));const world=new ResidentialDistrictWorld3D(game,canvas);return(await Promise.resolve(world.init()))?world:null}
...
async function boot(){
 ...
 if(!world&&gl){try{world=await startWebGL(game,canvas);if(world)backend="webgl"}catch(e){console.error("The Settlement: local WebGL residential presentation unavailable",e)}}
