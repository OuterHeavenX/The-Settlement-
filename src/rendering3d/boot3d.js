...
async function startWebGL(game,canvas){let GuardWorld3D;({GuardWorld3D}=await import("./GuardWorld3D.js"));const world=new GuardWorld3D(game,canvas);return(await Promise.resolve(world.init()))?world:null}
...
async function boot(){
 ...
 if(!world&&gl){try{world=await startWebGL(game,canvas);if(world)backend="webgl"}catch(e){console.error("The Settlement: local WebGL presentation unavailable",e)}}
