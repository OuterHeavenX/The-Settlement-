...
async function startWebGL(game,canvas){let HeroSliceWorld3D;({HeroSliceWorld3D}=await import("./HeroSliceWorld3D.js"));const world=new HeroSliceWorld3D(game,canvas);return(await Promise.resolve(world.init()))?world:null}
...
async function boot(){
 ...
 if(!world&&gl){try{world=await startWebGL(game,canvas);if(world)backend="webgl"}catch(e){console.error("The Settlement: local WebGL hero presentation unavailable",e)}}
 ...
 /* Preserve the already-established 3D input bridge; Three.js still does not
    own gameplay input semantics or placement authority. */
 ...
 console.info(`The Settlement: optional Three.js ${backend.toUpperCase()} presentation active`);
}
boot();
