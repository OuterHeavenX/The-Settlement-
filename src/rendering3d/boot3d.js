...
async function startWebGL(game,canvas){let AgricultureWorld3D;({AgricultureWorld3D}=await import("./AgricultureWorld3D.js"));const world=new AgricultureWorld3D(game,canvas);return(await Promise.resolve(world.init()))?world:null}
...
async function boot(){
 ...
 if(!world&&gl){try{world=await startWebGL(game,canvas);if(world)backend="webgl"}catch(e){console.error("The Settlement: local WebGL agriculture presentation unavailable",e)}}
