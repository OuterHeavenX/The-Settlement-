 ...
 * Important: Amenities override buildStructure() directly so they can never fall
 * through to the generic finished-building box while this renderer is active.
 */
...
export class AmenityPolishWorld3D extends PolishWorld3D{
 ...
 init(){const ok=super.init();if(!ok)return false;for(let i=0;i<10;i++){const l=new THREE.PointLight(0xffb45f,0,260,2);l.visible=false;this.scene.add(l);this.amenityLights.push(l)}console.info("The Settlement: bespoke Amenity 3D presentation ready");return true}
 ...
 makeAmenity(b){
  ...
  switch(v){
   case"lamp":box(5,48,5,iron,0,24,0);box(19,4,4,iron,7,47,0);box(15,18,15,warm,14,38,0);this.add(g,this.cyl(11,14,5),stoneDark,0,2.5,0);break;
   ...
   case"snackCart":box(40,24,34,wood,0,15,0);box(44,5,38,cloth,0,40,0);box(5,24,5,wood,-18,28,-14);box(5,24,5,wood,18,28,-14);this.add(g,this.cyl(9,9,5,10),iron,-17,5,18,Math.PI/2);this.add(g,this.cyl(9,9,5,10),iron,17,5,18,Math.PI/2);{const f=this.add(g,this.cyl(3,11,18,7),warm,12,35,0);f.userData.flame=true}break;
   ...
   case"brazier":this.add(g,this.cyl(14,20,10,10),iron,0,16,0);for(const x of[-10,10])box(3,15,3,iron,x,7.5,0);{const f=this.add(g,this.cyl(2,12,28,7),warm,0,35,0);f.userData.flame=true}break;
 ...
 updateAmenityLights(){
  ...
  for(let i=0;i<this.amenityLights.length;i++){const l=this.amenityLights[i],a=glows[i];if(i>=budget||!a){l.visible=false;continue}l.visible=true;l.position.set(a.x,a.type==="lamp"?72:a.type==="gazebo"?58:38,a.z);l.intensity=a.type==="brazier"?2.2:a.type==="lamp"?1.8:1.05;l.distance=a.type==="lamp"?310:250}
 }
