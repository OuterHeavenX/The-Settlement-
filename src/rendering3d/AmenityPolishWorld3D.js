/* Amenities presentation layer: bespoke low-poly civic props + pooled camera-near lights.
 * Presentation only. Placement/input/save/economy remain owned by existing systems.
 */
import * as THREE from "../../vendor/three/three.module.min.js";
import {PolishWorld3D} from "./PolishWorld3D.js";
const T=64;
export class AmenityPolishWorld3D extends PolishWorld3D{
 constructor(game,canvas){super(game,canvas);this.amenityLights=[];this.amenityGeo=new Map()}
 init(){const ok=super.init();if(!ok)return false;for(let i=0;i<10;i++){const l=new THREE.PointLight(0xffb45f,0,300,2);l.visible=false;this.scene.add(l);this.amenityLights.push(l)}console.info("The Settlement: bespoke Amenity presentation active");return true}
 ...
 buildStructure(b){const d=Settlement.BuildingDefs[b.type];if(!d?.amenity||!b.complete)return super.buildStructure(b);return this.makeAmenity(b)}
 makeAmenity(b){
  const d=Settlement.BuildingDefs[b.type],v=d.amenityMeta?.visualType,g=new THREE.Group(),w=b.w*T,h=b.h*T;
  const stone=this.mat("amenityStone",0x706d68),stoneDark=this.mat("amenityStoneDark",0x46464a),wood=this.mat("amenityWood",0x3f2d24),wood2=this.mat("amenityWood2",0x5a3c2b),iron=this.mat("amenityIron",0x25272b,{metalness:.32}),leaf=this.mat("amenityLeaf",0x3c4c35),leaf2=this.mat("amenityLeaf2",0x506044),flower=this.mat("amenityFlower",0x7d465b),water=this.mat("amenityWater",0x345363,{roughness:.28,metalness:.12}),warm=this.mat("amenityWarm",0xb07035,{emissive:0xffa34f,ei:2.3}),hot=this.mat("amenityHot",0xffb34f,{emissive:0xff7b22,ei:3}),roof=this.mat("amenityRoof",0x292e34),cloth=this.mat("amenityCloth",0x65283a),paper=this.mat("amenityPaper",0xc7b98e);
  const box=(W,H,D,M,x=0,y=0,z=0)=>this.add(g,this.box(W,H,D),M,x,y,z),tree=(x,z,s=1)=>{box(6*s,26*s,6*s,wood,x,13*s,z);this.add(g,this.sph(18*s),leaf,x,35*s,z);this.add(g,this.sph(13*s),leaf2,x-11*s,29*s,z+3*s)};
  switch(v){
   case"lamp":{box(5,48,5,iron,0,24,0);this.add(g,this.cyl(11,14,5,10),stoneDark,0,2.5,0);box(22,4,4,iron,8,47,0);box(15,18,15,iron,14,38,0);box(10,13,10,warm,14,38,0);box(18,3,18,iron,14,48,0);break}
   ...
   case"snackCart":box(40,24,34,wood,0,15,0);box(44,5,38,cloth,0,40,0);box(5,24,5,wood,-18,28,-14);box(5,24,5,wood,18,28,-14);this.add(g,this.cyl(9,9,5,10),iron,-17,5,18,Math.PI/2);this.add(g,this.cyl(9,9,5,10),iron,17,5,18,Math.PI/2);this.add(g,this.cyl(7,10,8,8),iron,12,29,0);{const f=this.add(g,this.cyl(2,8,18,7),hot,12,43,0);f.userData.flame=true}break;
   case"park":box(18,2,h-18,stoneDark,0,1,0);box(w-18,2,18,stoneDark,0,1,0);tree(-58,-55,.8);tree(58,-48,.75);tree(-54,55,.72);tree(55,55,.8);for(const[x,z]of[[-30,34],[-15,40],[26,-34],[38,-28]])this.add(g,this.sph(5,7,5),flower,x,5,z);box(55,7,16,wood2,0,12,58);break;
   ...
   case"brazier":this.add(g,this.cyl(13,18,9,10),iron,0,15,0);for(const x of[-9,9])box(3,14,3,iron,x,7,0);{const f1=this.add(g,this.cyl(2,11,27,7),hot,0,32,0);f1.userData.flame=true;const f2=this.add(g,this.cyl(1,7,20,6),warm,-4,35,2);f2.userData.flame=true}break;
 ...
 updateAmenityLights(){const tier=this.game.quality?.tier||"MEDIUM",budget={LOW:2,MEDIUM:4,HIGH:7,ULTRA:10}[tier]||4,phase=this.live?.phase?.(),night=phase==="NIGHT"||phase==="DUSK"||phase==="DAWN",cx=this.game.camera.x,cz=this.game.camera.y,glows=[];if(night)for(const b of this.game.buildings.list){const m=Settlement.BuildingDefs[b.type]?.amenityMeta;if(!b.complete||!m?.nightRelevant)continue;const x=(b.x+b.w/2)*T,z=(b.y+b.h/2)*T;glows.push({x,z,d:(x-cx)*(x-cx)+(z-cz)*(z-cz),type:m.visualType})}glows.sort((a,b)=>a.d-b.d);for(let i=0;i<this.amenityLights.length;i++){const l=this.amenityLights[i],a=glows[i];if(i>=budget||!a){l.visible=false;continue}l.visible=true;l.position.set(a.x,a.type==="lamp"?72:a.type==="gazebo"?58:38,a.z);l.intensity=a.type==="brazier"?2.2:a.type==="lamp"?1.8:1.05;l.distance=a.type==="lamp"?310:250}}
 animateAmenities(){const t=performance.now()/1000;for(const mesh of this.meshes.values()){if(!mesh?.userData?.amenity)continue;mesh.traverse(o=>{if(o.userData?.water){if(o.userData.baseY===undefined)o.userData.baseY=o.position.y;o.position.y=o.userData.baseY+Math.sin(t*2+mesh.userData.buildingId)*.18}else if(o.userData?.flame){o.scale.y=.9+Math.sin(t*8+mesh.userData.buildingId)*.18;o.rotation.y=t*2}else if(o.userData?.banner){o.rotation.y=Math.sin(t*1.6+mesh.userData.buildingId)*.08}})}}
