...
(()=>{
 ...
 for(const id of Object.keys(D)){
  ...
  for(let lv=1;lv<=MAX;lv++){
   const e=Object.assign({level:lv,name:label(d,lv),requiredTownLevel:d.id==="cottage"&&lv<=3?COTTAGE_TL[lv-1]:d.id==="mainHall"&&lv<=5?HALL_TL[lv-1]:townLevel(d,lv),buildTime:r((d.buildTime||10)*(1+.45*(lv-1))),xpReward:r((d.xpReward||10)*(1+.6*(lv-1))),cost:cost(d.cost,lv)},benefit(d,lv));
   levels.push(e);
  }
  d.levels=levels;d.maxLevel=MAX;if(d.id==="cottage")d.housingLevels=levels;
 }
})();
