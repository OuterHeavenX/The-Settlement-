Settlement.Renderer=class{
 ...
 building(b){let ctx=this.ctx,d=Settlement.BuildingDefs[b.type],x=b.x*64,y=b.y*64,w=b.w*64,h=b.h*64;if(!b.complete){ctx.fillStyle="#896944";ctx.fillRect(x+5,y+5,w-10,h-10);ctx.strokeStyle="#e5d2a0";ctx.strokeRect(x+8,y+8,w-16,h-16);ctx.fillStyle="#402c1d";ctx.fillRect(x+8,y+h-13,(w-16)*b.progress,6);ctx.fillStyle="#fff";ctx.font="16px serif";ctx.fillText("🔨",x+w/2-8,y+h/2);return}
  if(b.type==="farm"){let p=this.game.farms.normalize(b),crop=p.crop&&Settlement.CropDefs[p.crop];ctx.fillStyle="#6d4d2b";ctx.fillRect(x+4,y+4,w-8,h-8);for(let yy=0;yy<4;yy++)for(let xx=0;xx<4;xx++){ctx.strokeStyle="#4b331e";ctx.beginPath();ctx.moveTo(x+10+xx*28,y+8);ctx.lineTo(x+10+xx*28,y+h-8);ctx.stroke();if(crop&&p.state!=="empty"){let ready=p.state==="ready",growth=Math.max(.15,p.progress);ctx.fillStyle=ready?crop.color:crop.color+"cc";let px=x+16+xx*28,py=y+18+yy*25;ctx.beginPath();ctx.arc(px,py,3+5*growth,0,7);ctx.fill();if(p.tended){ctx.fillStyle="#b8d66d";ctx.fillRect(px-1,py-10*growth,2,7*growth)}}}return}
