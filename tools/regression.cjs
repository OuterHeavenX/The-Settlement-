...
(async()=>{
 ...
 const G=(fn,arg)=>page.evaluate(fn,arg),wait=ms=>page.waitForTimeout(ms);
 ...
 check('ALL BUILD CATEGORIES',total===await G(()=>Object.values(Settlement.BuildingDefs).filter(d=>!d.wonder&&(!d.requiredTownLevel||d.requiredTownLevel<=15)).length),`${total} cards`);
 await page.click('[data-act="cat:Wonders"]');await wait(80);check('WONDERS CATALOGUE VISIBLE',await G(()=>document.querySelectorAll('#buildmenu .wonder-card').length===4));check('LOCKED WONDERS REMAIN VISIBLE',await G(()=>document.querySelectorAll('#buildmenu .wonder-card .wonder-status').length===4&&document.querySelector('#buildmenu')?.textContent.includes('The Pantheon')));
 ...
 await page.click('[data-nav="BUILD"]');await page.click('[data-act="cat:Residential"]');await page.click('[data-build="cottage"]');await G(()=>window.game.placement.setPreviewTile(131,131));await page.click('[data-place-act="confirm"]');await wait(80);check('CONFIRM CREATES ONE',await G(n=>game.buildings.list.length===n+1,before.n),`before ${before.n}`);
 const hall=await G(()=>{let g=game,x=30,y=30,b=g.buildings.create('hallOfLegends',x,y);b.complete=true;b.progress=1;g.expansion.onBuildingComplete(b);let claimed=g.expansion.isClaimed(x,y),rects=g.expansion.claimedRects.length;g.buildings.demolish(b.id);return{claimed,rects,after:g.expansion.claimedRects.length,still:g.expansion.isClaimed(x,y)}});check('HALL CLAIM SOURCE OWNERSHIP',hall.claimed&&hall.after<hall.rects&&!hall.still,JSON.stringify(hall));
